...
class GeneralHelpers
{
    ...
    public static function followButton($following, $follower, $element, $size = ''){
        ...
        $relation = Following::where('following_user_id', $following)->where('follower_user_id', $follower)->get()->first();
        if ($relation){
            if ($relation->allow == 0) {
    ...
    public static function deniedButton($me, $follower, $element, $size = ''){
        ...
        $relation = Following::where('following_user_id', $me)->where('follower_user_id', $follower)->get()->first();
        if ($relation){
            if ($relation->allow == 1) {
    ...
    public static function notifications(){
        if (self::$notifications == null){
            ...
            $likes = Like::where('seen', 0)->with('user')->join('posts', 'posts.id', '=', 'post_likes.post_id')
